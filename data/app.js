...
async function scanDevices() {
    ...
    // ESP-NOW has a single control now — the persistent "Enable ESP-NOW"
    // toggle in the Network section. If it's on (and the device has since
    // restarted with it active), every scan quietly includes ESP-NOW; no
    // separate per-scan checkbox to keep in sync with it.
    const espnow = !!(CFG.network && CFG.network.espnow);
